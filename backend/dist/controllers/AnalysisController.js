...
const InterviewAnalysisService_1 = __importDefault(require("../services/InterviewAnalysisService"));
const pg_1 = require("pg");
class AnalysisController {
    constructor() {
        this.analyzeInterview = async (req, res) => {
            try {
            ...
            `;
                ...
                await this.pool.query('UPDATE interviews SET analysis_status = $1 WHERE id = $2', ['processing', interviewId]);
                const recordingPath = `uploads/${videoFile}`;
                const analysisResult = await this.analysisService.analyzeInterview(parseInt(interviewId), recordingPath, questions);
                await this.pool.query('UPDATE interviews SET analysis_status = $1, analysis_id = $2 WHERE id = $3', ['completed', analysisResult.id, interviewId]);
                res.status(200).json({
                    success: true,
                ...
                if (req.params.interviewId) {
                    await this.pool.query('UPDATE interviews SET analysis_status = $1 WHERE id = $2', ['failed', req.params.interviewId]);
                }
                res.status(500).json({
                    success: false,
        ...
        this.getAnalysisResult = async (req, res) => {
            try {
                const { interviewId } = req.params;
                const userId = req.user?.userId;
                if (!userId) {
        ...
        this.getUserAnalysisResults = async (req, res) => {
            try {
                const userId = req.user?.userId;
                if (!userId) {
                    res.status(401).json({ success: false, message: 'Unauthorized: User ID not found in token' });
        ...
        this.getAnalysisStats = async (req, res) => {
            try {
                const userId = req.user?.userId;
                if (!userId) {
                    res.status(401).json({ success: false, message: 'Unauthorized: User ID not found in token' });
        ...
        this.analysisService = new InterviewAnalysisService_1.default();
